...
We now present the design of \sys and its components (\autoref{fig:xsnare}). We begin by reviewing our threat model.
...
Our work makes no assumptions about the web server. In particular, the
server may run out of date and vulnerable software that delivers pages
to the user's browser with XSS exploits.

We trust the browser and the browser's extension mechanism to
correctly execute \sys. We also depend on the browser to disallow
malicious tampering with the client-side signature database.

We trust the analyst who writes the signature definitions used by
\sys. For \sys to be effective the signatures must be correct. However,
a signature that fails to match a vulnerability, will
only impact the page with longer load times.

\subsection{Overview}

We now review the high-level operation of \sys with \autoref{fig:xsnare}.
A user requests a page, \textit{example.com}, on a
...
HTML context can be used to defend against XSS, taken from CVE
...
server-side using a taint-tracking, so that a
...
This is straightforward once the fix is understood.
...
\sys does not automatically determine the actions to
implement from a patch. We assign this task to a security
analyst, who acts as the signature developer for an
exploit. The system automates signature matching and
sanitization.
...
We believe CVEs are an ideal source of signature
definitions. Previous client-side work does not benefit from our level
of specificity; these tools often use less accurate
heuristics to detect exploits. %% Furthermore, once new
%% vulnerabilities are found, these systems lack the
...
%
\sys signatures are written specifically to match a known software 
vulnerability. Even though they cannot be generated 
automatically, any trusted developer can write and compose 
these signatures. This does not require participation from application developers.


In general, we do not require the existence of a publicly disclosed CVE to be able to write a signature for an exploit.
CVEs have been useful to us as we did not discover the exploits.
% As described in \autoref{viability}, CVEs are a convenient way for us to test our system against real-world vulnerabilities. %However, 
...
The challenge of automatically deriving signatures from detailed CVEs is challenging, 
...
%manifests in the HTML, as well as the fix.
 ...
 Our signature language needs enough power of
 ...
 (For an in-depth description of the signature language, please see \cite{pazos2020precise}):
...
%in Section \ref{case_study}.
 In summary, a signature will have the
...
which takes care of common sanitization needs~\cite{safecontent}. However, DOMPurify's defaults can be unnecessarily restrictive, or not restrictive enough, in which case the other sanitization methods are preferable.
...
% See ~\autoref{appendix:language_specification} for
%more details.
...
by virtue of the DOM parsing. The extension will not find an injection in the expected
place, creating a false
...
of circumvention techniques for our detector, so we must analyze the DOM before the
 browser has a chance to modify it.
...
template. The content in between (i.e. the star) is an injection point ,
...
additional malicious content in between. If just searching for multiple
...
from the bottom-up) and sanitize everything in between.
...
non-malicious content, the black pluses (+). Since we know the order in which the
\code{endPoints} should appear, when the filter sees a closing
...
In both scenarios, we have to sanitize
the outermost end points. This might get rid of a substantial amount of valid HTML,
so we defer to the signature developer's judgment of what behavior the 
detector should follow. We expand upon this further in \autoref{case_study}.

Note that these complex cases do not mean that our approach is not applicable, 
as the extension provides a choice for blocking the page entirely if the signature
writer believes a given case is too complex for our signature language.
...
vulnerability is CVE-2018-7747 (in WordPress Caldera Forms, which allows malicious
content retrieved from the plugin's database to be injected in response to a click.

\sys allows XMLHTTPRequests (XHR) to be filtered with \js{XHR}-type
...
Listing 2 shows an example of such a signature. The idea is extensible
