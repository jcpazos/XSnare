...
Previously, we stated that our signatures include versioning, and our approach ideally works for specific version of the software, since once patched, the rate of false positives will increase severely. In many cases, this check can be done easily, as the vulnerable page will often have the plugin name and version number as part of the WordPress header or footer HTML. Unfortunately, we have found that, more often that not, this information is not available, either due to developer negligence or because they believe hiding the version number grants them more security. However, since our use-case is mostly for site administrators and privileged users, the extension can generate a quick request to the wp-admin/plugins.php page to verify that the page is indeed running the suspected plugin on a vulnerable version.
