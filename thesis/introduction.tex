...
If a patch for a server-side vulnerability can be ``translated'' into an equivalent patch for the output it generates (i.e. the page received by the browser), then a great opportunity arises. Our extension, which has access to the user's browsing context, can identify pages on which the patch should be applied based on a database of signatures. \sys can thus protect users as soon as a patch is implemented and added to its database. Furthermore, the client-side patch will continue to be beneficial until \textit{all} server operators running that software have had a chance to upgrade their deployments. We detail its design mechanisms in Section~\ref{xsnare_design}.
