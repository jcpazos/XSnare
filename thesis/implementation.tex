...
\begin{algorithm}
	\SetAlgoLined
	//global DBSignatures\;
	\SetKwProg{verifyRequest}{Function \emph{verifyRequest}}{}{end}
	\verifyRequest{(requestString, url, tabId)}{
	 loadedProbes = runProbes(HTMLString, url, domain)\;
	 signaturesToCheck = []\;
	 \For{probe in loadedProbes}{
  	 ...
  	 \For{signature in signaturesToCheck} {
  	 	\uIf{signature applies to HTMLString and url}{
  	 		loadedSignatures.push(signature)\;
  	 	}
  	 }
     versionInfo = loadVersions(domain, loadedProbes)\;
     endPointsList = [] \;
     \For{signature in loadedSignatures} {
     	\uIf{(signature,signature.version) $\in$ versionInfo}{
     		endPointsList.push(signature.endPoints)\;
     	}\uElseIf{versionInfo == null}{
    		//couldn't apply versioning, push anyway\;
    		endPointsList.push(signature.endPoints)\; 		
     	}
    } 
	...
	\For{endPointPair in endPointsList} {
		indices = findEndPointsIndices(HTMLString, endPointPair)\;
		sanitize(HTMLString,indices)\;
	}
		\caption{Request verification algorithm}
	}
\end{algorithm}
