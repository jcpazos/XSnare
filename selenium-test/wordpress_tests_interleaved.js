...
async function initTests(url) { 
	...
	try {
	 	...
	 	await warm_extension_driver.get("http://localhost:8080/wp-admin");
	 	await warm_extension_driver.findElement(By.id('user_login')).sendKeys('root');
	 	await sleep(1000);
 		await warm_extension_driver.findElement(By.id('user_pass')).sendKeys('root');
 		await sleep(1000);
 		await warm_extension_driver.executeScript('document.getElementById("loginform").submit()');
 		await sleep(1000);
	 	...
	 	await warm_no_extension_driver.get("http://localhost:8080/wp-admin");
	 	await warm_no_extension_driver.findElement(By.id('user_login')).sendKeys('root');
	 	await sleep(1000);
 		await warm_no_extension_driver.findElement(By.id('user_pass')).sendKeys('root');
 		await sleep(1000);
 		await warm_no_extension_driver.executeScript('document.getElementById("loginform").submit()');
 		await sleep(1000);
	} catch (err) {
	...
	stream = fs.createWriteStream("./results_wordpress_interleaved/extension_cold_top_results.txt", {flags:'a'});
	stream.write(JSON.stringify(loadTimesColdExtension));
	stream.write(",");
	stream.end();

	stream = fs.createWriteStream("./results_wordpress_interleaved/no_extension_cold_top_results.txt", {flags:'a'});
	stream.write(JSON.stringify(loadTimesColdNoExtension));
	stream.write(",");
	stream.end();

	stream = fs.createWriteStream("./results_wordpress_interleaved/extension_warm_top_results.txt", {flags:'a'});
	stream.write(JSON.stringify(loadTimesWarmExtension));
	stream.write(",");
	stream.end();

	stream = fs.createWriteStream("./results_wordpress_interleaved/no_extension_warm_top_results.txt", {flags:'a'});
